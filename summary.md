...
## 24. Version 0.10.0 - User-Friendly Plex Integration

This version dramatically improved the security and usability of the Plex integration by moving all configuration into the UI.

- **In-App Plex Authentication**: Implemented a secure, PIN-based OAuth flow. Users can now link their Plex account by visiting `plex.tv/link` with a code generated by the app, removing the need to manually manage authentication tokens.
- **Dynamic Library Selection**: The "Options" tab was enhanced to dynamically fetch and display a list of a user's available Plex libraries as checkboxes, preventing configuration errors.
- **Centralized Plex Configuration**: All Plex settings (URL, Token, and monitored libraries) are now stored in the database and managed entirely through the dashboard UI.
- **Simplified Setup**: The `PLEX_URL`, `PLEX_TOKEN`, and `PLEX_LIBRARIES` environment variables were removed, streamlining the initial setup process for new users.