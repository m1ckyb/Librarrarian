# Copilot Instructions for Librarrarian

This document provides guidelines for GitHub Copilot when working on the Librarrarian project.

---

## Session Initialization

When starting a new session, read the following files to understand the project's current state, architecture, and purpose:
...
This ensures all responses are informed by the full project context.
...
1. **Continuous Documentation**: After every feature addition, change, or bug fix, `unreleased.md` **must** be updated immediately with a concise summary of the change under the appropriate heading (`### Added`, `### Changed`, `### Fixed`). This ensures the changelog is always ready for the next release.
   - **Exception**: Changes made to `remember.md` or this file (`copilot-instructions.md`) do not need to be documented in `unreleased.md`.
...
- **Database Name Consolidation**: The worker (`worker/transcode.py`) currently defaults to the database name `transcode_cluster`, while the dashboard (`dashboard/dashboard_app.py`) defaults to `librarrarian`. This should be unified to `librarrarian` in a future release. This change was temporarily reverted to maintain compatibility with existing setups.

---

## Agent Capabilities & Limitations

- **Shell Access**: Copilot does not have direct access to the user's shell. It cannot run commands like `docker-compose`, `git`, or other command-line tools itself. It must always ask the user to run these commands and provide the output if needed.
