...
- **Show Ignored Toggle Not Hiding Items**: Fixed the "Show Ignored" toggle not actually hiding items set to "None (Ignore)" in the Plex and Internal Scanner monitored libraries/folders lists. The issue was caused by Bootstrap's `d-flex` class using `display: flex !important`, which overrode the inline `display: none` style. Now uses Bootstrap's `d-none` class to properly hide ignored items.
